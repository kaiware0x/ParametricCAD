...
/* XPM */
static const char* not_found[] = {"24 24 2 1",
                                  "# c #000000",
                                  ". c #ffffff",
                                  "........................",
                                  "........................",
...
//         if (!px.isNull()) icon.addPixmap(px);
//     }
    ...
    if (icon.isNull()) {
        ...
        if (!loadPixmap(fileName, icon)) {
            ...
            for (QList<QByteArray>::iterator fm = formats.begin(); fm != formats.end(); ++fm) {
                QString path =
                    QString::fromLatin1("%1.%2").arg(fileName, QString::fromLatin1((*fm).toLower().constData()));
                                         ...
                                         const std::map<unsigned long, unsigned long>& colorMapping) const
{
    ...
    QString iconPath;
    QString fn = QString::fromUtf8(name);
    if (QFile(fn).exists()) iconPath = fn;
...
//         if (!names.contains(item)) names << item;
//     }
//     return names;
// }
...
//        if (pix.width() == 0 || pix.height() == 0) return pix;    // do not resize a null pixmap
...
{
    QBitmap b = p.mask();
    if (b.isNull()) return p;    // sorry, but cannot do anything
...
//    overlayedIcon.addPixmap(Gui::BitmapFactory().merge(base.pixmap(w, w, QIcon::Normal, QIcon::Off), px, position),
//                            QIcon::Normal, QIcon::Off);

//    overlayedIcon.addPixmap(Gui::BitmapFactory().merge(base.pixmap(w, w, QIcon::Normal, QIcon::On), px, position),
//                            QIcon::Normal, QIcon::Off);
